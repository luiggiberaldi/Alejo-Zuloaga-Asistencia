...
|-------|--------|-------------|-----------|-----------|
| 2026-07-04 | Fix de bugs de Android (PDF/XLS) + crash nativo + UX de asistencia + Bitácora | Corregidos: importación XLS (fetch/Blob), compartir PDF en Android, logo roto en PDF (resize), crash nativo de Expo Go (versiones de reanimated/gesture-handler/worklets) — **confirmado resuelto en dispositivo**; agregado auto-scroll y modal de finalización en toma de asistencia, auto-pull al login, selector de fecha nativo en Reportes, modal estilizado para guardar/compartir PDF (reemplaza Alert nativo); commiteado todo el trabajo de la sesión en 4 commits separados por objetivo | Probar con un profesor real y una sección completa (hoy solo hay datos de prueba: 1 sección, 30 estudiantes); limpiar archivos sueltos en la raíz del repo (PDF de prueba, `logo.png` duplicado, `.vscode/`); revisar `.claude/settings.json` modificado (origen desconocido) | El crash de Expo Go tomó varias iteraciones diagnosticar correctamente (se confundió inicialmente con el trabajo de `react-native-paper-dates`, que ya se había revertido); Bitácora no se había actualizado desde el inicio del proyecto |
